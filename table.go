...
// IndexExpr declares a named index over SQL expressions written verbatim into
// the index key list; wrap an expression when the database demands it and
// append what belongs outside the parentheses, like an operator class in
// "lower(email) text_pattern_ops". Exception: MySQL requires every functional
// key part parenthesized, so each expression gains one pair there.
