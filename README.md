...
| Tamper detection | Each applied migration records a checksum of its compiled SQL and arguments (type-tagged and length-prefixed; pointer arguments hash by value). On drift, `Up` warns (or fails under `WithStrictChecksum`), `Status` reports it, and `Repair` re-records after review. The encoding changed in v0.10 — records written by earlier versions report drift once; run `Repair` after reviewing `Status`. |
